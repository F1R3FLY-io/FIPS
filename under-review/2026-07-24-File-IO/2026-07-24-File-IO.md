...
| `revoke()` | `[true]` | Turn off ambient authority (see §Revocation). |
...
- Post-`revoke`, every method above returns `[false, "FSERR_REVOKED", "Fs has been revoked"]`; see §Revocation.
...
`Fs.revoke()` turns off ambient authority.  Its purpose is to let a genesis-time powerbox contract acquire the specific `File`/`Dir` caps it wants to distribute per-principal, then close the door behind itself — so the same `Fs` cap can no longer serve as an ambient authority for opening new files.

**Semantics.**

- **Global one-way switch.**  `revoke()` on ANY `Fs` instance flips a shared, per-shard flag from `false` to `true`.  There is no un-revoke.  Idempotent — a second `revoke()` call also returns `[true]`.
- **All instances see the switch.**  Once flipped, every `Fs` instance in the shard (including instances that predate the `revoke()` call) rejects `openFile` / `openDir` / `stdin` / `stdout` / `stderr` with `[false, "FSERR_REVOKED", "Fs has been revoked"]`.  This is not per-cap-instance state; the switch is at the module level, shared across every `Fs` a powerbox may mint.
- **Post-revoke mints are dead-on-arrival.**  Any subsequent `Fs` constructor invocation (e.g., a per-principal mint attempted after the switch flipped) yields an instance whose methods all return `FSERR_REVOKED`.  The mint itself does not fail — Rholang agent constructors do not return values to the caller — but the resulting cap is inert.
- **Previously-minted `File` / `Dir` caps are unaffected.**  They are independent agents; `Fs`'s state change does not cascade.  A `File` acquired pre-`revoke` continues to read, write, seek, close, etc.  This is exactly the property that lets a powerbox capture caps before revoking.

**Ocap discipline.**

- `revoke()` is a method on the `Fs` cap itself — anyone holding an `Fs` cap can call it.  The design assumes the powerbox holds the only `Fs` cap during the genesis handoff window and voluntarily revokes at the end of its capture phase; after that, deploys can still `getFs`, but the resolved cap only exposes the always-erroring surface.
- The one-way property is intrinsic to the switch (Rholang tuplespace: once written, the cell's value can only be advanced forward by `revoke()`; there is no `unrevoke()`).
- Even the `revoke()` cap holder cannot re-enable ambient authority.  If a validator's operator wants to change the static bundle, the correct path is a hard-fork event (shard-wide re-genesis with a new bundle), not a live re-enable.

**Interaction with membranes.**

`FSERR_REVOKED` also arises from user-space membrane revocation (§Ocap patterns).  The two uses are semantically consistent: both signal "a cap that used to work no longer does."  A subordinate seeing `FSERR_REVOKED` from a membrane-wrapped `File` handles it the same way as one seeing `FSERR_REVOKED` from a post-revoke `Fs.openFile` call — the recovery path is application-specific (re-request from grantor, escalate, give up).

**Consensus.**

The revoke flag is Rholang tuplespace state and is therefore consensus-observable: every validator sees the same `revoke()` transition at the same block height.  A Consensus-mode deploy attempting `Fs.openFile("consensusPath")` after `revoke` sees `FSERR_REVOKED` on every validator identically.  No new hard-fork surface beyond the new `FSERR_REVOKED` error code; the WAL / reply-hash verification path is unchanged (a `revoke`-blocked `openFile` never journals because its handler layer isn't reached).

**Not for stopping I/O generally.**

`revoke()` is specifically the ambient-authority off-switch.  Actual on-disk I/O by deploys that already hold `File`/`Dir` caps continues normally.  For per-file access-control ratcheting, use range locks (§Range locks) or membranes.  For shard-wide filesystem shutdown, that is an operator-level shard-halt concern outside this FIP.
...
**Concurrency partition.**  `readInto`/`readAtInto`/`writeFrom`/`writeFromAt` are byte-level and take the same implicit range locks as `bytesAt`/`writeBytesAt`; `readLineInto`/`readLinesInto` are line-based and hold the whole-file cursor.  All six accept the optional `{"wait": true}` options-map argument — the arity-N+1 form `(<primary-args>, {"wait": Bool})` where a missing key or `Nil` value defaults to `false`.  (Errata 2026-09-02: earlier drafts said "all five", off-by-one on `readLinesInto`.)
...
| `removeDir(rel, options)` | `[true, nDeleted]` | Delete a directory.  `options.recursive: true` (default `false`) uses `remove_dir_all`; `false` requires the directory to be empty and returns `"FSERR_IO"` otherwise.  `nDeleted` is the number of filesystem entries removed (see §`removeDir` reply shape). |
...
`removeDir` returns a per-entry-count `nDeleted` on every code path so callers, cost accounting, and per-validator observation all agree on the size of the operation.

- **Non-recursive success**: `[true, 1]` — the target directory itself was removed (removeDir requires the directory to be empty in this mode, so exactly one filesystem entry is deleted).
- **Recursive success (Oracular or Consensus)**: `[true, nDeleted]` where `nDeleted` is the total number of filesystem entries removed by the recursive walk — the target plus every descendant file, subdirectory, and other entry the walk touched.
- **Partial-failure**: `[false, code, msg, nDeletedBeforeError]` — the count of entries actually removed before the first error terminated the walk.  A caller observing `[false, ...]` still knows the exact scope of the effect.  For non-recursive removeDir, `nDeletedBeforeError` is always `0` (the target itself failed).

Under consensus mode, recursive removeDir additionally carries a per-entry manifest as an implementation-side channel for follower re-execution — the manifest is used by the follower's re-execute-and-verify path (§Consensus vs. oracular) but is not part of the observable reply shape to Rholang callers; `Dir.removeDir` unwraps the transport shape and exposes only `[true, nDeleted]` upward.  A Consensus caller cannot distinguish a Consensus removeDir reply from an Oracular one by inspecting the reply Par.

*Rationale.*  Every prior variant of the reply shape either omitted the count (`[true]` — leaves callers blind, allows under-charging on Oracular recursive) or exposed a per-entry manifest at the Rholang layer (`[true, [[path, kind], ...]]` — inconsistent between recursive/non-recursive and between cmodes; forces every caller to pattern-match on the shape).  A single `nDeleted` integer on every success path gives cost accounting, retention accounting, audit trails, and library authors the same clean contract regardless of `recursive` or `cmode`.
...
**Atomicity scope for aggregating methods** (2026-08-12 refinement, added during slice 8a step 4e implementation).  Two sequential-write methods delegate internally to other sequential-write methods and inherit their atomicity granularity accordingly:

- `writeString(s)` delegates to `writeByteArray(s.toUtf8Bytes())` — atomicity is per-call (one atomic `writeByteArray` covers the entire String).
- `writeLines(lineStream)` iterates the outer LineStream and delegates each inner CharStream to `writeLine(inner)` — atomicity is **per-line, not per-writeLines**.  Between two internal `writeLine` calls, another sequential caller (from another File cap on the same physical file, or from another concurrent branch on this cap) can acquire the whole-file sequential lock, emit its own bytes, release, and get out of the way before this cap's next writeLine.  For cross-call atomic multi-line writes (all lines land contiguously without another writer interleaving), callers must arrange coordination at a wider scope than the writeLines call.  Under the same-holder-skip rule of §Range locks, an explicit `lockRange` held via the SAME cap as `writeLines` conflicts with each internal writeLine's sequential acquire (sequential is strict same-holder, unlike positional) — the workaround requires a DIFFERENT cap on the same `(dev, inode)` holding the outer range lock to block cross-cap sequential attempts while this cap runs writeLines.
