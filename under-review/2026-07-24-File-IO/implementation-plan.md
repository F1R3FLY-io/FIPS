...
- **WAL fresh-tree applier: sequential-`Write` reconstruction** (surfaced 2026-08-26 during Path A(ii) implementation).  The `apply_wal_to_fresh_tree` helper in `rholang/tests/fs_wal_spec.rs` (landed 2026-08-26 alongside `pb_m_14_file_state_identity_via_wal_replay`) supports `WriteAt` + `Truncate` today but panics on sequential `WalOp::Write` with a pointer to this gap.  Root cause: `WalOp::Write` carries `path` + `length` + `payload_ref` but no fd identity, no file-position, and no `Open`/`Close`/`Seek` sibling entries — so a stand-alone applier can't determine where the bytes landed.  Two paths to close: (a) add `Open`/`Close`/`Seek` variants to `WalOp` and thread the appends through `fs_open`/`fs_close`/`fs_seek` handlers, letting the applier simulate fd-position evolution per file; (b) promote `Write` to always carry an absolute offset in the WAL entry (making it structurally identical to `WriteAt`).  Impact scope: only relevant for non-consensus tooling that reconstructs filesystem state from WAL alone without re-executing Rholang (Phase 7b joiners fall back to this when `write-payload determinism` doesn't apply — see plan §377).  Consensus-static content authored via `fsWriteAt` avoids the gap entirely; the applier restriction is enforced with a loud panic so a future slice that closes the gap will trip the applier's arm and prompt an extension.
